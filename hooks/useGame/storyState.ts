...
const 默认开局时间 = '1:01:01:00:00';
const 规范化开场开局时间 = (value?: string): string => normalizeCanonicalGameTime(value || '') || 默认开局时间;

export const 创建开场空白环境 = (startTime = 默认开局时间): 环境信息结构 => ({
    时间: 规范化开场开局时间(startTime),
    大地点: '',
    中地点: '',
    小地点: '',
    具体地点: '',
    节日: null,
    天气: { 天气: '', 结束日期: 规范化开场开局时间(startTime) },
    环境变量: []
});
...
export const 创建开场基础状态 = (charData: 角色数据结构, worldConfig: WorldGenConfig, openingConfig?: OpeningConfig) => {
    const 开局时间 = 规范化开场开局时间(openingConfig?.自定义开局时间);
