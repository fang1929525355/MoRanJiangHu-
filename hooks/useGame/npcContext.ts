...
export const 提取NPC生图基础数据 = (npc: any, options?: 生图基础数据选项) => {
    ...
    const 读取装备衣着 = (source: any): string => {
        const equip = source?.当前装备;
        if (!equip || typeof equip !== 'object') return '';
        const parts = ['服装', '内衣', '内裤', '袜饰', '鞋履', '饰品']
            .map((slot) => {
                const value = equip[slot];
                const name = typeof value === 'string' ? value.trim() : (value && typeof value.名称 === 'string' ? value.名称.trim() : '');
                return !name || name === '无' ? '' : name;
            })
            .filter(Boolean);
        return parts.join('，');
    };
    const 衣着文本 = 读取首个文本字段(npc, ['衣着风格', '衣着', '衣着要点']);
...
export const 提取NPC香闺秘档部位生图数据 = (npc: any, part: '胸部' | '小穴' | '屁穴' | '肉棒', options?: 生图基础数据选项) => {
    ...
    const 描述字段 = part === '胸部' ? '胸部描述' : part === '小穴' ? '小穴描述' : part === '肉棒' ? '肉棒描述' : '屁穴描述';
