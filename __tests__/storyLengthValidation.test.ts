    ...
    it('酒馆预设使用 cot 占位符时仍保留时间和地点关键运行规则', () => {
        ...
        const builtContext: 主剧情系统上下文 = {
            ...
            contextPieces: {
                ...
                女主剧情规划状态: '',
                世界状态: '',
                环境状态: '当前环境：大唐 / 长安 / 客栈 / 前厅；时间：1:01:01:08:00',
                角色状态: '',
                战斗状态: '',
                门派状态: '',
                任务状态: '',
            ...
            gameConfig: {
                ...
                酒馆预设列表: [{
                    ...
                    预设: {
                        spec: 'chara_card_v3',
                        prompts: [
                            { identifier: 'main', name: 'main', role: 'system', content: '固定预设 {{cot}}' },
                            { identifier: 'worldInfoBefore', name: 'worldInfoBefore', role: 'system', content: '' },
                            { identifier: 'userInput', name: 'userInput', role: 'user', content: '' }
                        ],
                        prompt_order: [{
        ...
        const combined = result.orderedMessages.map((message) => message.content).join('\n\n');
        expect(combined).toContain('每回合必须根据正文推进 `环境.时间`');
        expect(combined).toContain('地点变化必须有正文事实依据');
        expect((combined.match(/每回合必须根据正文推进 `环境\.时间`/g) || []).length)
